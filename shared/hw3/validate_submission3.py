...
def get_submitters(temp_folder):    
    submitters_path = os.path.join(temp_folder, 'submitters.txt')
    if not os.path.exists(submitters_path):
        raise Exception('Unable to find submitters.txt - Invalid format')

    required_files = ['vegenere.h', 'vegenere.c']
    for f in required_files:
        if not os.path.exists(os.path.join(temp_folder, f)):
            raise Exception('No %s file - Invalid format' % f)
    
    sub_text = submitters_path
